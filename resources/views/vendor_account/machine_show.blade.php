  ...
  <div class="card-body p-0">
    <ul class="list-group list-group-small list-group-flush cap-text">
      ...
      <li class="list-group-item d-flex px-3">
        ...
        <span class="ml-auto text-right text-semibold text-reagent-gray">{{$machine->maintainance == 0 ? 'Yes' :
          'Live'}}</span>
      </li>
      <li class="list-group-item d-flex px-3">
        <span class="text-semibold text-fiord-blue">Status</span>
        <span class="ml-auto text-right text-semibold text-reagent-gray">{{$machine->active == 0 ? 'Not active' :
          'active'}}</span>
      </li>
    </ul>
  </div>
      ...
      <ul class="nav row">
        <li class="nav-item ml-auto">
          <a class="btn btn-sm btn-info"
            href="{{route('vendor_account_machine_edit', [$vendor->id, $machine->id])}}">Edit
          </a>
        </li>
        <li class="nav-item ml-auto">
          @if ($machine->active == 1)
          <a class="btn btn-sm btn-danger" role="menuitem"
            href="{{ route('vendor_account_machine_activation_toggle', [$vendor->id, $machine->id]) }}"
